...
CACHE_FILE = Path(tempfile.gettempdir()) / f".claude_context_cache_{_PROJECT_HASH}.json"
CACHE_TTL = 300
...
def run_context_monitor() -> int:
    ...
    if percentage >= THRESHOLD_WARN:
        ...
        print(
            f"{YELLOW}   Complete current task, then use SlashCommand tool to run /remember{NC}",
            file=sys.stderr,
        )
