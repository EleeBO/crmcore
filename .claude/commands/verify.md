---
description: Verify completed implementation against plan
model: opus
---
# VERIFY MODE: 10-Step Verification and Quality Assurance

## The Process

Tests → Program execution → Feature parity → Call chain analysis → Coverage → Quality checks → Code review → E2E → Final verification → Status update
...
```bash
uv run pytest tests/unit/ -v  # Python
# or
npm test                       # TypeScript
```
...
```bash
uv run pytest tests/integration/ -v  # Python
# or
...
1. Store findings in `.claude/memory/` (write a summary file)
...
### Step 4: Feature Parity Check (for migrations/refactoring)
...
1. **Store findings in project memory:**
   ```
   Write findings to .claude/memory/verification-FEAT-NNN.md
   ```
...
4. **Inform user:**
   ...
   2. Run `/implement <plan-path>` to implement missing features
   3. Run `/verify <plan-path>` again after implementation
   ```
...
```bash
uv run pytest --cov=src --cov-report=term-missing  # Python
# or
...
- [ ] **Convention Compliance:** Follows project standards (see coding-standards.md)
...
**When applicable:** Web frontends, user-facing features

**Options (in priority order):**
1. Agent Browser: `just e2e <url>` — quickest
2. Chrome MCP: `mcp__claude-in-chrome__*` tools — interactive
3. Playwright: `npx playwright test` — automated
...
**If failures:** Analyze failure → Fix implementation → Re-run → Continue until all pass

### Step 10: Final Verification & Status Update
