...
**Spec statuses:** DRAFT → ACTIVE → MODIFIED → DEPRECATED → ARCHIVED

| Transition | Trigger | Who Updates |
|------------|---------|-------------|
| → DRAFT | /specify creates new spec | /specify command |
| DRAFT → ACTIVE | /implement completes (archive step) | /implement command |
| ACTIVE → MODIFIED | /specify updates existing spec | /specify command |
| MODIFIED → ACTIVE | /implement completes (archive step) | /implement command |
| ACTIVE → DEPRECATED | Manual decision | User |
| DEPRECATED → ARCHIVED | Move to specs/archive/ | User |

**Registry (`specs/registry.md`) MUST be updated on every status change.**
